...
<div class="container max-w-screen-sm" id="buscontent">
    <form id="formjadwaledit" method="post">
...
?>><?php echo $val['nama']; ?></option>
        ...
        <div class="mb-3 flex flex-col ">
            ...
            <input value="<?=$row['time_start']?>" name='time_start' type="text" id='time' placeholder='Jam' class="p-2" />
        </div>


        <div class="mb-3 flex flex-col ">
            <label for="time" class="form-label">Perkiraan Waktu Tempuh</label>
            <input type="number" name='pwt' id="pwt" placeholder='Jam' class="p-2" value="<?=$row['pwt']?>" />
        </div>
...
<script>
 $('#formjadwaledit').validate({ // initialize the plugin
        rules: {
            busjadwal: {
                required: true,
            },
        ...
        submitHandler: function(form,e) {
            ...
            $.ajax({
                type: 'POST',
                url: '/transgo/components/admin/jadwal/proses.php?c=edit&id=' + id,
                dataType: "html",
                data: $('form').serialize(),
